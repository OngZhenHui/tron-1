...
public abstract class bikeMovement {
	...
	public bikeMovement(int x, int y, int velocityX, int velocityY, int width,
						int height) {
		this.x = x;
		this.y = y;
	 ...
	 * 
	 */
	public collision intersects(bikeMovement other) {
		if (other != this) {
		...
		ArrayList<lightCycleBike> pa = other.getPath();
		for (int i = 0; i < pa.size() - 1; i++) {
			lightCycleBike k = pa.get(i);
			int x1 = k.getStartX();
			int y1 = k.getStartY();
			int x2 = k.getEndX();
			int y2 = k.getEndY();
	...
	public abstract ArrayList<lightCycleBike> getPath();
}
